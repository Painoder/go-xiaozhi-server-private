...
type BizConf struct {
	...
	GLMRealtime  GLMRealtimeConf `yaml:"glm_realtime"`
...
func GLMRealtimeConfig() *GLMRealtimeConf {
	return &conf.GLMRealtime
}
...
func (c *BizConf) Validate() error {
	...
	if c.Provider.Name == "glm-realtime" {
		if c.GLMRealtime.APIKey == "" {
			return fmt.Errorf("glm_realtime.api_key is required")
		}
		if c.GLMRealtime.BaseURL == "" {
			return fmt.Errorf("glm_realtime.base_url is required")
		}
	}
	if c.Xiaozhi.Format == "" {
