...
import (
	...
	"github.com/xdimtech/go-xiaozhi/pkg/protocol/glmrealtime"
...
func NewXiaozhiHandler(ctx context.Context, conn *websocket.Conn) (*XiaozhiHandler, error) {
	sess := NewApiSession(ctx, config.GLMRealtimeConfig().Model)
...
func (r *XiaozhiHandler) handleHelloEvent(ctx context.Context,
	event *xiaozhi.ClientEventHello) (any, error) {
	...
	glmConf := config.GLMRealtimeConfig()
	systemPrompt := glmConf.SystemPrompt
	maxToken := openai.IntOrInf(1024)
	eventID := utils.UniqueID()

	chatMode := glmConf.ChatMode
	if chatMode == "" {
		chatMode = "audio"
	}

	glmSession := glmrealtime.GLMClientSession{
		Model:             glmConf.Model,
		...
		Voice:             lo.ToPtr(openai.Voice(glmConf.Voice)),
		...
		BetaFields: &glmrealtime.GLMBetaFields{
			ChatMode: chatMode,
		},
	}

	pbEvent := &glmrealtime.GLMSessionUpdateEvent{
...
func (r *XiaozhiHandler) handleListenEvent(ctx context.Context,
	event *xiaozhi.ClientEventListen) (any, error) {
	return nil, nil
}
...
func (r *XiaozhiHandler) handleIotEvent(
	ctx context.Context, ev *xiaozhi.ClientEventIot) (any, error) {
	return nil, nil
}
